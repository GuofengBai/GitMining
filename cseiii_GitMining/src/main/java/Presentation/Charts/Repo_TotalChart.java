...
public class Repo_TotalChart
  extends ApplicationFrame
{
  public Repo_TotalChart(String paramString)
  {
    super(paramString);
  ...
  private static TableXYDataset createDataset()
  {
    ...
    String sql = "SELECT tsum FROM gitmining.repository_commits order by tsum;";
    String str = "";
    Statement pstmt;
    int num = 0 ;
    int width = 0;
    try {
        ...
        rs.next();
        for(width=0;width<600000;width+=5000){
        	num = 0;
        	for(;rs.getInt("tsum")<=width;rs.next()){
        		num++;
        	}
        	localXYSeries1.add(width, num);
        }
        num=0;
        for(;rs.getInt("tsum")<38000000;rs.next()){
        	num++;
        }
        localXYSeries1.add(600001, num);
        localXYSeries1.add(610000, num);
  ...
  {
	  JFreeChart localJFreeChart = ChartFactory.createXYAreaChart("Repo_TotalChart", "Total", "Count", paramTableXYDataset, PlotOrientation.VERTICAL, true, true, false);
  ...
  {
	  Repo_TotalChart localStackedXYAreaRendererDemo1 = new Repo_TotalChart("StackedXYAreaRendererDemo1");
